...
export async function addUser(req: AddUserRequest, reply: FastifyReply) {
    if (await PRISMA.user.findFirst({ where: { id: req.body.userId } })) {
        return reply.status(400).send({ message: RESPONSE_TOPICS.USER_ALREADY_EXISTS });
    }
...
export async function setActivity(req: SetActivityRequest, reply: FastifyReply) {
    if (!await PRISMA.user.findFirst({ where: { id: req.body.userId } })) {
        return reply.status(404).send({ message: RESPONSE_TOPICS.USER_NOT_FOUND });
    }
